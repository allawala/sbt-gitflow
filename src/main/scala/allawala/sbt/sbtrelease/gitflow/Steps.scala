...
object Steps {
  ...
  private type ReleaseStep = State => State

  private def run(fn: GitFlow => Either[String, Unit]): ReleaseStep = { state =>
    (for {
      git <- getGit(state)
      _ <- fn(GitFlow(git))
  ...
  private def run(fn: (GitFlow, String) => Either[String, Unit]): ReleaseStep = { state =>
    (for {
      git <- getGit(state)
